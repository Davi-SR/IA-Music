...
class StemMixer {
  ...
  play() {
    ...
    const position = this.currentTime() >= this.duration - 0.1
      ? 0
      : this.currentTime();
    const attempt = (this.playAttempt || 0) + 1;
    this.playAttempt = attempt;
    this.playing = true;
    this.updatePlayButton();
    this.setStatus("Iniciando sua mixagem…");
    ...
    for (const track of this.tracks) {
      ...
      if (Number.isFinite(track.audio.duration)) {
        track.audio.currentTime = Math.min(position, track.audio.duration);
      }

      Promise.resolve(track.audio.play())
        .then(() => {
          if (
            this.destroyed ||
            !this.playing ||
          ...
          this.setStatus(
            startedTracks === this.tracks.length
              ? "Reproduzindo todas as faixas em sincronia."
              : "Reproduzindo a mixagem enquanto as outras faixas entram.",
          );
        })
        .catch(() => {
          ...
          failedTracks += 1;
          this.renderTrackState(track);
          if (failedTracks === this.tracks.length) {
            this.pause();
            this.setStatus(
    ...
    window.setTimeout(() => {
      if (
        ...
        this.setStatus(
          "As faixas ainda estão preparando o primeiro trecho. A reprodução começará automaticamente.",
        );
      }
    }, 2500);
  }
  ...
  correctDrift(referenceTime) {
    for (const track of this.tracks) {
      if (
        !track.audio.paused &&
        track.audio.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA &&
  ...
  currentTime() {
    const activeTrack = this.tracks.find(
      (track) =>
        !track.audio.paused &&
        !track.audio.ended &&
        track.audio.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA,
    );
    if (activeTrack) return activeTrack.audio.currentTime;
    return Math.max(
      0,
      ...this.tracks
        .map((track) => track.audio.currentTime)
  ...
  renderTrackState(track) {
    ...
    row.classList.toggle(
      "is-muted",
      track.unavailable || track.muted || (hasSolo && !track.solo),
    );
