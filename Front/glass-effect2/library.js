...
class StemMixer {
  ...
  async initialize() {
    ...
    const results = await Promise.allSettled(
      this.tracks.map((track) => waitForMetadata(track.audio)),
    );
    ...
    if (!durations.length) {
      throw new Error("As faixas não puderam ser carregadas para reprodução.");
    }
    ...
    this.setStatus("Tudo carregado. Dê play e monte sua própria mixagem.");
    this.applyVolumes();
  }
...
function waitForMetadata(audio) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = window.setTimeout(
      () => reject(new Error("Tempo excedido ao carregar uma faixa.")),
      30000,
    );
    ...
    audio.addEventListener(
      ...
      () => {
        window.clearTimeout(timeout);
        reject(new Error("Falha ao carregar uma faixa."));
      },
      { once: true },
    );
    audio.load();
